...
\begin{tikzpicture}[scale=0.65, every node/.style={font=\small}]
    ...
    % Backward arrows
    \draw[->,red,dashed,thick] (L) to[bend right=30] node[below]{\scriptsize $\frac{\partial L}{\partial a}$} (a);
    \draw[->,red,dashed,thick] (a) to[bend right=30] node[below]{\scriptsize $\frac{\partial a}{\partial z}$} (z);
    \draw[->,red,dashed,thick] (z) to[bend right=30] node[left]{\scriptsize $\frac{\partial z}{\partial w}$} (w);
